...
def validate(values: dict[str, str], *, mode: str, now: datetime | None = None) -> None:
    if mode not in {"template", "readiness", "qualification"}:
        raise AuthorizationError("unsupported D4 authorization mode")
    if values["INPUT_VERSION"] != "portal.execution-d4.owner-input.v1":
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument(
        "--mode", choices=("template", "readiness", "qualification"), required=True
    )
