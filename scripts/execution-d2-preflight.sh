...
required=(
  ...
  EDGE_SOURCE_PROBES_ENABLED EDGE_MANAGER_V2_READ_ENABLED
  EDGE_SOURCE_CLIENT_IDENTITY_FILE EDGE_SOURCE_API_KEY_FILE
  EDGE_PROJECTION_INGESTION_ENABLED EDGE_REALTIME_SSE_ENABLED
  EDGE_ANALYTICS_QUERY_ENABLED EDGE_ANALYTICS_SOURCE_PROFILE
  EDGE_SHADOW_QUERY_ENABLED EDGE_PAPER_WORKBENCH_SHADOW_ENABLED EDGE_COMMAND_RELAY_ENABLED
)
...
local_edge_pattern='^portal-execution-edge-manager-v2@sha256:[a-f0-9]{64}$'
[[ "${values[PORTAL_SOURCE_PROXY_IMAGE]}" =~ ${image_pattern} ]] || {
  printf 'D2 preflight requires an immutable GHCR digest for PORTAL_SOURCE_PROXY_IMAGE.\n' >&2
  exit 1
}
if [[ "${values[PORTAL_EXECUTION_EDGE_IMAGE]}" =~ ${image_pattern} ]]; then
  [[ "${values[EDGE_DEV_LOCAL_IMAGE_ALLOWED]}" == false ]] || {
    printf 'Signed Edge images must not enable the dev-local image exception.\n' >&2
    exit 1
  }
else
  [[ "${values[EDGE_DEV_LOCAL_IMAGE_ALLOWED]}" == true &&
     "${values[PORTAL_EXECUTION_EDGE_IMAGE]}" =~ ${local_edge_pattern} ]] || {
    printf 'D2 preflight requires a signed GHCR Edge digest or the explicit dev-local content address.\n' >&2
    exit 1
  }
fi
...
edge_private_port="${values[EDGE_PRIVATE_PORT]:-8443}"
[[ "${edge_private_port}" =~ ^[0-9]{4,5}$ ]] || {
  printf 'D2 preflight rejected the Edge private listener port.\n' >&2
  exit 1
}
(( edge_private_port >= 1024 && edge_private_port <= 65535 )) || {
  printf 'D2 preflight requires an unprivileged Edge private listener port.\n' >&2
  exit 1
}
...
if [[ "${manager_extension_set}" == eds11r-r4-r5 ]]; then
  ...
  expected_admission_file="/run/secrets/event-ledger-runtime-${values[EDGE_ENVIRONMENT]}.json"
  expected_state_suffix="/event-ledger/${values[EDGE_ENVIRONMENT]}"
  [[ "${values[EDGE_MANAGER_EVENT_LEDGER_ADMISSION_FILE]:-}" == "${expected_admission_file}" &&
     "${values[EDGE_EVENT_LEDGER_STATE_DIRECTORY]:-}" == /* &&
     "${values[EDGE_EVENT_LEDGER_STATE_DIRECTORY]}" != *'/../'* &&
     "${values[EDGE_EVENT_LEDGER_STATE_DIRECTORY]}" == *"${expected_state_suffix}" ]] || {
    printf 'EDS-11R5 requires profile-isolated admission and state paths.\n' >&2
    exit 1
  }
  if [[ "${mode}" =~ readiness$ ]]; then
    [[ "${values[EDGE_EVENT_LEDGER_STATE_DIRECTORY]}" == "/srv/primus/portal/runtime${expected_state_suffix}" ]] || {
      printf 'EDS-11R5 readiness requires the canonical private state directory.\n' >&2
      exit 1
    }
  fi
fi
...
[[ "${values[EDGE_PROJECTION_INGESTION_ENABLED]}" == false &&
   ...
   "${values[EDGE_SHADOW_QUERY_ENABLED]}" == false &&
   "${values[EDGE_PAPER_WORKBENCH_SHADOW_ENABLED]}" == false &&
...
if [[ -n "${values[SOURCE_PROXY_MANAGER_LOCATIONS_FILE]:-}" ]]; then
  [[ "${values[SOURCE_PROXY_MANAGER_LOCATIONS_FILE]}" == /* &&
     "${values[SOURCE_PROXY_MANAGER_LOCATIONS_FILE]}" != *'/../'* ]] || {
    printf 'D2 preflight rejected an unsafe Manager locations path.\n' >&2
    exit 1
  }
fi
...
if [[ "${mode}" != template ]]; then
  ...
  if [[ "${manager_source_enabled}" == true ]]; then
    manager_locations_file="${values[SOURCE_PROXY_MANAGER_LOCATIONS_FILE]:-${values[SOURCE_PROXY_SECRET_DIRECTORY]}/manager-v2-locations.conf}"
    check_secret "${manager_locations_file}" 640
