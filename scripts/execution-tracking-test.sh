...
python3 - "${MASTER}" "${TRACKER}" "${ROADMAP}" "${LEDGER}" "${BACKEND_README}" <<'PY'
from pathlib import Path
import re
import sys

master, tracker, roadmap, ledger, backend = [Path(p) for p in sys.argv[1:]]
for path in (master, tracker, roadmap, ledger, backend):
    if not path.is_file():
