...
required=(
  INPUT_VERSION OWNER STORAGE_APPROVED AWS_INSTANCE_ID AWS_AVAILABILITY_ZONE
  AWS_VOLUME_ID
...
[[ "${values[AWS_AVAILABILITY_ZONE]}" =~ ^ap-east-1[a-z]$ ]] || {
  printf 'D4 storage preflight rejected the AWS availability zone.\n' >&2
  exit 1
}
