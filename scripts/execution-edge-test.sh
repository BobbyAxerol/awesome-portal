...
PROJECTION_SIGNATURE_SQL="SELECT concat((SELECT count(*) FROM _sqlx_migrations), ':', (SELECT count(*) FROM portal_projection.epochs), ':', (SELECT count(*) FROM portal_projection.event_journal), ':', (SELECT count(*) FROM portal_projection.analytics_source_snapshots), ':', (SELECT count(*) FROM portal_projection.analytics_source_facts), ':', (SELECT count(*) FROM portal_projection.d4_source_checkpoints), ':', (SELECT count(*) FROM portal_projection.d4_source_failures), ':', (SELECT count(*) FROM portal_projection.shared_consumer_leases), ':', (SELECT count(*) FROM portal_projection.manager_projection_leases), ':', (SELECT count(*) FROM portal_projection.manager_projection_heartbeats), ':', (SELECT count(*) FROM portal_projection.manager_projection_commits), ':', (SELECT count(*) FROM portal_projection.manager_projection_cycles), ':', (SELECT count(*) FROM portal_projection.manager_projection_cycles WHERE realtime_sequence IS NOT NULL), ':', (SELECT count(*) FROM portal_projection.retention_lifecycle_policy_snapshots), ':', (SELECT count(*) FROM portal_projection.retention_recovery_checkpoints), ':', (SELECT count(*) FROM portal_projection.retention_cleanup_runs), ':', (SELECT count(*) FROM portal_projection.shadow_screen_activations), ':', (SELECT count(*) FROM portal_projection.source_admission_state), ':', (SELECT count(*) FROM portal_projection.source_admission_leases), ':', (SELECT count(*) FROM portal_projection.source_read_cache), ':', (SELECT count(*) FROM portal_projection.authoritative_event_streams), ':', (SELECT count(*) FROM portal_projection.authoritative_event_generations), ':', (SELECT count(*) FROM portal_projection.authoritative_event_batches), ':', (SELECT count(*) FROM portal_projection.authoritative_event_facts), ':', (SELECT count(*) FROM portal_projection.authoritative_event_current), ':', (SELECT count(*) FROM portal_projection.authoritative_event_local_journal), ':', (SELECT count(*) FROM portal_projection.authoritative_event_quarantines));"
...
printf 'Execution edge contracts, auth, transport, bounded load, N24 three-profile durable projection, replay/query, retention/recovery/cleanup, qualification, source-backed analytics, adapter rollback and PostgreSQL restore gates passed.\n'
