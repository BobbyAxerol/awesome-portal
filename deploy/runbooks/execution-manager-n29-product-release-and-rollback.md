...
3. Require the N29 verdict to remain
   `RELEASE_CANDIDATE_READY_PROTECTED_RELEASE_PENDING`. BR-EX-72 and the
   same-origin frontend consumer are accepted and digest-bound; only protected
   publication evidence may advance the release verdict.
4. Merge through `dev`; promote a reviewed commit to `main` only after the
   protected workflow produces the exact signed candidate evidence.
