...
`portal/` is the parent integration project for independently versioned child
repositories. It builds their checked-out source into one deployable service
stack; developers do not need to start and manually connect each application.

## Current stack

| Source repository | Services exposed by the parent stack |
| --- | --- |
...
Open `http://localhost:8080`. The public web service proxies `/api` to the
internal API service, so all portal components use a single deployment entry
point. Use `./scripts/portal logs`, `./scripts/portal status`, and
...
`repos.conf` is the workspace manifest. To clone/fetch every configured child
...
`repos.lock` pins the exact child commit used for CI and image publishing. To
promote a deliberately chosen child revision after its own repository has been
...
To integrate a future sub-portal:
...
For private child repositories, provide a `PORTAL_REPOS_TOKEN` repository
