...
- **EDS-09 Trading System owner-return mirror (COMPLETED, docs-only):**
  ...
  checkout. The Portal now advances through the separately approved EDS-09b
  observation-revision, EDS-10b observed/derived-panel and EDS-11 local-SSE
  sequence; authoritative Event ingestion remains unavailable unless a later
  owner return contains an independently verified `EVENT_SOURCE_ACCEPTED`
  class. See
...
- **EX-DP-01/E2 deployed truth, census and semantic audit closed
  ...
  application relations / 1,387 columns, 96 source-baseline objects plus three
  deployed runtime overlays. E2 then applied a separate owner-only, bounded
  read-only Rust semantic audit: all 99 relations readable, 11/11 domains
  classified and 48/48 fixed planner probes captured. Its no-local-role
  negative failed closed; the private evidence records authority, identity,
  source-clock, retention and correction truth without exporting rows or
  payloads. The observed plans are 5 index-backed and 43 sequential, so E3
  must freeze each screen/resource/cursor before any typed return is exposed;
  a generic relation reader remains forbidden. There is still no Portal DB
  client, row export, route, cache, command, Event/SSE or deployment
  activation. See EX-BE-02C in the Unified Plan for private evidence digests,
  source-owner gaps and the unchanged reversible two-relation read ACL.
...
### 14.1.9 PR #64 composed-smoke image reliability repair (2026-09-12)

The final CI smoke failure was isolated to composition bootstrap, not product
logic or the visual application. Docker Hub no longer resolves the historical
MinIO image, while the immutable identical OCI index remains available from
Quay. Local/CI and the future stable overlay pin that index by digest. The
smoke script now builds each owned producer image before `up --no-build`; this
prevents `quant-worker-py`, which correctly shares the API image but has no
independent build stanza, from racing an unavailable CI-only image pull. A
focused static test locks both invariants. The stable overlay source change is
not a deploy or restart, and no frontend, command or external-source authority
