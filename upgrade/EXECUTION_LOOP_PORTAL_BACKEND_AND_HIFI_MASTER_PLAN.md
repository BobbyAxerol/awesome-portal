...
| PRE-IAM-04 | Offline operational hardening | close confirmed H-series contract/security issues, load/replay/restore/rollback evidence that needs no AWS source | consume only published fixtures/contracts; keep failure states visible | focused regression + workspace gates; credential-free evidence report | `INTEGRATION_COMPLETE / PRODUCTION_INACTIVE` — requalified with 39/39 contracts, 149/149 Control API and 95 Rust/PG tests on 2026-08-23 |
| PRE-IAM-05 | Prepare D2 dark deployment | image/config/service manifests for AWS Portal Edge + Source Proxy with interfaces/routes/source reads still off | no AWS/live UI activation; continue fixture work | offline render, non-root/read-only image checks, preflight and rollback rehearsal | `INTEGRATION_COMPLETE / PRODUCTION_INACTIVE` — renderer-owned runtime group and render→ownership→readiness order requalified 2026-08-23; D2 remains unauthorized |
| PRE-IAM-06 | Tracking cleanup | reconcile Master Plan, backend README/guide, shared tracker and request ledger; classify every blocker by owner | reconcile FE status/evidence and close superseded requests | no contradictory status, no bare `COMPLETE`, exact next dependency/owner per row | `INTEGRATION_COMPLETE / PRODUCTION_INACTIVE` — seven documentation authorities plus generated catalogue are drift-gated 2026-08-23 |
...
`PRE-IAM-04` closes only the credential-free operational hardening lane. It
adds loss-detectable gap/cursor contracts, typed analytics problems, bounded
ledger/funnel windows with exact global facts, six-screen fixture/schema/serde
parity, pinned-adapter rollback protection and executable PostgreSQL restore
...
The 64-entry canonical `noun/verb` catalogue revision 2 is generated from the immutable
Trading System contract pack with source digests; all entries remain
`portal_reachable=false`. It is ADMIN/workspace/actor/environment/entity/risk
scoped; observed non-GET routes are at least R1 with owner review and all R1–R4
items require plan/apply. The eight unpublished `ops` actions and generic Redis
access remain blocked. TypeScript creates only immutable blocked plans with
bounded sensitive-key rejection and `HASH_ONLY_NO_RAW` retention,
