...
|---|---|---|
| 2026-09-03 | **Tail-follow expiry loop found and root-caused across cells**: facade page cursors expire after 5 minutes; at a stream tail no fresh cursor is ever minted, so the held cursor rots -> `CURSOR_EXPIRED` -> SGP wiped the accumulated window and restarted the full oldest-first backfill (observed 09:01Z on `performance_snapshots` right after its window first filled with 2,000 points). Trading-side fix committed on HK branch `fix/manager-cursor-ttl-tail-follow` (`99515e8`, TTL 48 h, deploy awaits owner rule-12 approval; rollback image `2e80ad38`); SGP-side no-wipe carry-forward is in codex's working tree (uncommitted — left untouched). BR-EX-79 gains item 1.4. Numeric-id history fix (`f4b6fb2`) verified live: history store at 220k+ equity / 129k performance rows (performance history COMPLETE to source tail 08-17) | worker logs (ladder_drained 10k rows/cycle, relation_failed 09:01Z), facade code line `exp=iat+5min`, HK commit `99515e8` |
