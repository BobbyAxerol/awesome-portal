...
### 7.9 BR-EX-67 — governance review evidence & policy-verdict data (filed 2026-08-30)

The owner's 2026-08-30 hi-fi copies of Gate R1/R2 add panels the disk corpus did not have —
R2's **Gate criteria — policy vs evidence** table and **Stage eligibility** chips — and the R1
evidence charts stop being honest-state frames. All ship as labeled smoke in
`governance.smoke.ts`; the shapes there are this row's reference.
...
```jsonc
{
  ...
  "evidence_series": {           // chart-series.rules.v1 in full
    "run_id": "run_5512",
    "roles": [                    // window roles fixed by the methodology claim
      { "role": "IS",      "points": [["2019-01-01", "1.0000"], ...] },
      { "role": "OOS",     "points": [...] },
      { "role": "HOLDOUT", "points": [...] }
    ],
    "boundaries": [{ "role": "OOS", "at": "2022-05-01" }, { "role": "HOLDOUT", "at": "2024-09-01" }],
    "max_drawdown": { "at": "2023-05-01", "value": "1.3810", "pct": "-11.4" },  // rule 6
    "wfo": { "threshold": "1.0", "median": "1.34", "min": "0.71", "dispersion": "PASS",
...
```jsonc
{
  "portfolio_fit": {             // labeled research estimates, replaced by Paper observation
    "target_weight_pct": "8.0",
    "corr_estimate": { "value": "0.18", "window": "90d backtest", "formula_version": "corr.v1" },
    "expected_marginal_risk_pct": "+5.2", "diversification_benefit": "+0.07",
    "symbol_overlap": "NONE_WITH_LIVE"
  },
  "gate_criteria": {
    "policy": { "policy_id": "gate_r2", "rev": 7, "effective": "2026-07-01", "declared_by": "Risk admin" },
    "rows": [{ "criterion": "Sharpe (net, 1y)", "threshold": ">= 1.20", "observed": "1.74",
               "verdict": "PASS" }, ...],   // verdict server-computed; browser renders only
...
**Invariants.** Thresholds and verdicts are policy data from the versioned gate-policy records —
admin-declared, and a rev change re-evaluates open requests server-side; the portal never
computes a verdict, and a criterion evaluated against a superseded rev arrives as
`STALE_POLICY`, not as a silently recomputed PASS. The evidence series obey
`chart-series.rules.v1` — the max-drawdown annotation equals the series value at its bucket
(rule 6), window roles are fixed by the methodology claim, gaps stay gaps.

**DoR:** amended schemas + fixture variants + error/absent examples before route work.
**Frontend retires on delivery:** `governance.smoke.ts`, the SMOKE notes on both screens,
re-record the r1/r2 baselines; honest not-published paths stay for the absent case.
...
| 2026-08-30 | Claude: §7.2 BR-EX-69/70/71 appended (`RECEIVED`) + spec §7.11 — the owner-commissioned governance additions: loop-entry create-request, live-gate canary payload, fleet-wide conditions register; all three screens live as declared demos same-day | documentation only; no runtime/profile/source/command change |
| 2026-08-30 | Claude: **UI/UX VERSION FREEZE declared by the owner** — no new screens, flows or backend requests will be filed for this version; the complete backend scope to CLOSE it is: registry items (frontend HOTFIX_REQUEST_2026-08-30 §1–§2.1), the two fixture-route fixes (handoff §8.34), BR-EX-41…71 (§7.2) and the OHLC owner decision (§7.5.5(1)). Each delivery removes its declared smoke block on the frontend; compositions do not change. Closure conditions: frontend ROADMAP_FRONTEND §J.3 | documentation only; no runtime/profile/source/command change |
