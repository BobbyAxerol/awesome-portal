...
| — | Codex N02 incremental source contract | `PORTAL_REQUEST_VERIFIER_COMPLETE / OWNER_PACK_PENDING / RUNTIME_V1_LOCKED` — request-only v2 schema + exact four-file digest envelope + 15-case fail-closed verifier; read-only discovery chưa thấy owner-published v2 | Claude dùng typed gap/resync/retention/completeness fixtures, giữ Lane B fixture/unavailable; không chọn live reader trước owner acceptance + N03/N04 | `EX_BE_02_N02_INCREMENTAL_SOURCE_CONTRACT_REVISION.md`, `CODEX_TO_CLAUDE_N02_INCREMENTAL_SOURCE_CONTRACT_HANDOFF.md` |
| — | Codex N03 owner implementation gate | `PORTAL_ACCEPTANCE_HARNESS_COMPLETE / N02_OWNER_PACK_PENDING / OWNER_IMPLEMENTATION_PENDING / RUNTIME_V1_DORMANT` — exact five-file evidence chain, immutable commit/image, zero-idle/no-full-scan/query-bound verifier 15/15; không sửa/call Trading System | Claude chỉ tiếp tục fixture/parity typed states; không chọn Lane B trước N02+N03 owner acceptance và N04 Rust consumer | `EX_BE_02_N03_TRADING_SYSTEM_INCREMENTAL_SOURCE_IMPLEMENTATION.md`, `CODEX_TO_CLAUDE_N03_OWNER_IMPLEMENTATION_HANDOFF.md` |
...
| — | Codex N06 real-source qualification | `PORTAL_QUALIFICATION_AUTHORITY_COMPLETE / REAL_24H_EVIDENCE_PENDING / SOURCE_DARK` — exact N02/N03/image/schema binding, parity, 12 drills and bounded 24h evidence; acceptance can never activate registry/runtime | Claude may use sanitized template/state fixtures; never label synthetic or candidate evidence as source active | `EX_BE_03_N06_REAL_SOURCE_QUALIFICATION_AND_SOAK.md`, `CODEX_TO_CLAUDE_N06_REAL_SOURCE_QUALIFICATION_HANDOFF.md` |
| — | Codex N07 Paper Workbench shadow APIs | `PORTAL_IMPLEMENTATION_COMPLETE / SHADOW_PROFILE_INACTIVE / N06_OWNER_PROMOTION_PENDING` — manifest-bound atomic epoch activation; deployment-scoped Rust orders/positions Query; exact counts/aggregates, signed keyset, freshness/retention; TS session+mTLS/H2+JWT BFF; all flags false | Claude wires only generated `orders`/`positions` adapter + honest states/parity; registry stays fixture; BR-EX-41/49/50 wider panels remain pending | `EX_BE_03_N07_PROJECTION_QUERY_ANALYTICS_SHADOW.md`, `CODEX_TO_CLAUDE_N07_SHADOW_SCREEN_APIS_HANDOFF.md` |
...
| 4 | Paper Workbench (1c) | **screen built** (fixtures, scale-refined) | `PORTAL_IMPLEMENTATION_COMPLETE / PRODUCTION_INACTIVE` | Claude consumes N07 orders/positions adapter; real N06 evidence + owner shadow promotion remain | N07 deployment-scoped orders/positions API, manifest-bound ACTIVE authority, exact counts/aggregates and typed states delivered; registry fixture/flags off |
