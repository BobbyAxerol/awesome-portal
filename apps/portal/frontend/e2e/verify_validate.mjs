...
const tw = await page.evaluate(() => {
  ...
  return { passBadges: (body.match(/schema|boundaries|content hash/g) || []).length, hasError: body.includes("Có lỗi xảy ra") || body.includes("Internal Server Error") };
});
...
try {
  ...
  const aw = await page.evaluate(() => {
    const body = document.body.innerText;
    return { hasError: body.includes("Có lỗi xảy ra") || body.includes("Internal Server Error") || body.includes("500"), hasBars: body.includes("bars") };
  });
