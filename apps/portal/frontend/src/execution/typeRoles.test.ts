...
describe("the locked role scale", () => {
  ...
  it("never applies uppercase outside the table-header role", () => {
    ...
    const offenders = rules
      ...
      .filter(([, sel, body]) => /text-transform\s*:\s*uppercase/.test(body) && !/--exec-font-th\)/.test(body) && !/\.exec-role-th/.test(sel) && !/^\s*\.exec-(cc|inc2|oq|bl|af|a3|rp|pf2|ab|bd|ac|lv|lf|cn|sbc|sb|360|surface)[-\s]/.test(sel))
