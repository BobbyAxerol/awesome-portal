        ...
        <tbody>
          {aggregates.map((a) => (
            <tr key={a.currency} data-invalid={(a.invalidNumericCount ?? 0) > 0 ? "true" : undefined}>
              ...
              <td className="exec-num">{a.rowCount ?? <span className="exec-role-meta">count not published</span>}</td>
              <td className="exec-num">{a.quantity ?? "not published"}</td>
              <td className="exec-num">{a.quantityCount ?? <span className="exec-role-meta">count not published</span>}</td>
              <td className="exec-num">{a.notional ?? "not published"}</td>
              <td className="exec-num">{a.notionalCount ?? <span className="exec-role-meta">count not published</span>}</td>
              <td className="exec-num">
                {a.invalidNumericCount === null ? <span className="exec-role-meta">not published</span> : a.invalidNumericCount > 0 ? <span className="exec-chip" data-tone="bad">{a.invalidNumericCount} unparsable</span> : "0"}
              </td>
            </tr>
          ))}
        </tbody>
