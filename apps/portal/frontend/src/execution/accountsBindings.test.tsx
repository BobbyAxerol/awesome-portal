...
import { ACCOUNTS_SMOKE_DATA } from "./accounts.smoke";
...
describe("Binding Detail (smoke until BR-EX-53)", () => {
  it("draws the capital invariant bar with one segment per virtual account and the headroom edge", () => {
    const { container } = render(<BindingDetail bindingId="binance_main_01" />);
  ...
  it("an unknown binding is an honest unavailable state", () => {
    render(<BindingDetail bindingId="nope" />);
    expect(screen.getByText(/No binding detail is published for nope/)).toBeTruthy();
  });
});
