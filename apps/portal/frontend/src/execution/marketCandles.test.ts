...
describe("market candles reader", () => {
  it("keeps prices as the venue's strings and drops a malformed bar", () => {
    ...
    expect(p.coverage).toEqual({ fromMs: T0, toMs: T0 + 7_199_999, requestedLimit: 500, returnedCount: 2, truncated: false });
    expect(readMarketCandles({ state: "READY" })).toBeNull();
  });
  it("builds the query with the fixed venue/market and a clamped limit", () => {
    expect(marketCandlesPath({ symbol: "ETHUSDT", interval: "15m", fromMs: 1.4, toMs: 2, limit: 9000 })).toBe("/market/candles?venue=BINANCE&market=USDM&symbol=ETHUSDT&interval=15m&from_ms=1&to_ms=2&limit=1500");
  });
  it("steps the interval up until the range fits one venue page", () => {
    expect(fittingInterval(3 * 86_400_000, "1h")).toBe("1h");
    expect(fittingInterval(90 * 86_400_000, "1h")).toBe("4h");
    expect(fittingInterval(2 * 86_400_000, "1m")).toBe("5m");
    expect(fittingInterval(400 * 86_400_000, "1h")).toBe("1d");
  });
});
