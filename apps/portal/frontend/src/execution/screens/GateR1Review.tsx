...
export function GateR1Review({
  ...
  const locked = effectiveLocks.length > 0 || Boolean(decisionsBlockedReason);
        ...
        actions={
          isDecided ? null : (
            <>
              ...
              title={approveLocked ? (approveReasons.join(" · ") || DECISION_CLOSED) : undefined} onClick={onApprove}>
                Approve
              </button>
            </>
          )
